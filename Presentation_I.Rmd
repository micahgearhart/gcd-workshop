...
> - Pupae Formation Example
> - 2014 World Cup Example
...
```{r Pupae Formation 14,eval=F}
TukeyHSD(aov(Pupation_Rate ~ genotype + diet,dwa))
```
Genotype:
```{r Pupae Formation 15,echo=F}
TukeyHSD(aov(Pupation_Rate ~ genotype + diet,dwa))$genotype
```
## ANOVA
```{r Pupae Formation 16,eval=F}
TukeyHSD(aov(Pupation_Rate ~ genotype + diet,dwa))
```
Diet:
```{r Pupae Formation 17,echo=F}
TukeyHSD(aov(Pupation_Rate ~ genotype + diet,dwa))$diet
```
...
## EXERCISE 5 - Answers
```{r Excercise 5  Answers 1} 
head(cup) 
```

## EXERCISE 5 - Answers
```{r Exercise 5 Answers 2}
max(cup$age)
min(cup$age)
```

## EXERCISE 5 - Answers
```{r Excercise 5 Answers 3}
tapply(cup$age,cup$country,median)
```
...
## EXERCISE 7 - ANSWERS
```{r EXERCISE 7 Answers} 
ggplot(cup,aes(x=country,y=age)) + geom_boxplot()+ theme_bw() +
   theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
   ggtitle("Ages of World Cup 2014 by Country")
```
